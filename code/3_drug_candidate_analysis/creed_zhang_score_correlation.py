...
df = pd.read_csv("../../result/drug_result/cmap_zhang_score.csv", index_col="sig_id")
t2t = {"colorectal cancer":"COADREAD"}
...
network = map(lambda x: x, network)
NETWORK = network
...
for net in NETWORK:
    ...
    for cc in TYPE:
        ...
        #net_score = ps.loc[dmap.loc[zha_score.index,"drug"], t2t[can]]
        ...
        pearson.append(stats.pearsonr(zha_score, net_score)[0])
    re_s[net] = spearman
    re_p[net] = pearson

re_s = pd.DataFrame(re_s, index=l1000.keys())
re_p = pd.DataFrame(re_p, index=l1000.keys())
re_s.to_csv("../../result/drug_result/l1000_spearman_zhang_score_cell_median.csv")
re_p.to_csv("../../result/drug_result/l1000_pearson_zhang_score_cell_median.csv")